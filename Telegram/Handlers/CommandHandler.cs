...
public class CommandHandler : BaseHandler
{
    ...
    public async Task HandleAsync(Message message, CancellationToken ct)
    {
        ...
        {
            _logger.LogDebug(BotConstants.SystemMessages.CommandNoText, message.From?.Id);
            return;
        }
    ...
    private async Task HandleGroupCommandAsync(Message message, long userId, CancellationToken ct)
    {
        if (!IsAdmin(userId))
        {
            _logger.LogWarning(BotConstants.SystemMessages.AdminNoPermissionCreate, userId);
            return;
        }
    ...
    private async Task FinalizeGroupCreationAsync(Message message, Player player, string groupName, CancellationToken ct)
    {
        ...
        _logger.LogInformation(
            BotConstants.SystemMessages.GroupCreatedLog,
            player.TelegramId,
            groupName,
            message.Chat.Id);
